...
public class GameScene: SKScene, SKPhysicsContactDelegate {
 ...
 it will perform the contact and collision. */
    ...
    public func createBulletsClone(_ name: String) {
        // Bullet Sprite Node
        bulletClone = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: name)))
        bulletClone.zPosition = -5
        // Bullet Physics Body Properties
        bulletClone.size = CGSize(width: size.width * 0.007 * (sizeBullet / 100), height: size.width * 0.028 * (sizeBullet / 100))
